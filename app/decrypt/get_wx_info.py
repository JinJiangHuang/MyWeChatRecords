...
"""
class Wechat来源：https://github.com/xaoyaoo/PyWxDump/blob/master/pywxdump/wx_info/get_wx_info.py#L34
"""
...
def get_info_name(h_process, address, address_len=8, n_size=64):
    ...
    info_name = get_info_without_key(h_process, address1, n_size)
    if info_name != "None":
        return info_name
    array = bytes(array).split(b"\x00")[0] if b"\x00" in array else bytes(array)
    text = array.decode('utf-8', errors='ignore')
...
error_wechat_no_run = "[-] WeChat No Run"
...
def read_info(version_list: dict = None, is_logging: bool = False, save_path: str = None):
    ...
    for process in wechat_process:
        ...
        if not isinstance(bias_list, list) or len(bias_list) <= 4:
            ...
            rd['key'] = get_key(rd['filePath'], addrLen)
            if rd['key'] == 'None':
        ...
        if rd['wxid'] != "None" and rd['filePath'] == "None":  # 通过wxid获取filePath,如果filePath为空则通过wxid获取filePath
            rd['filePath'] = get_info_filePath(rd['wxid'])
...
def load_wx_config():
    ...
    if not args.vlfile and not args.vldict:
        VERSION_LIST_PATH = get_resource_path(os.path.join('app', 'decrypt', 'version_list.json'))
