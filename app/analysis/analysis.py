...
def wordcloud_(wxid, time_range=None):
    ...
    if not os.path.exists(stopwords_file):
        ...
        stopwords_file = os.path.join(resource_dir, 'app', 'stopwords.txt')
    with open(stopwords_file, "r", encoding="utf-8") as stopword_file:
...
def get_wordcloud(text):
    ...
    if not os.path.exists(stopwords_file):
        resource_dir = getattr(sys, '_MEIPASS', os.path.abspath(os.path.dirname(__file__)))
        stopwords_file = os.path.join(resource_dir, 'app', 'data', 'stopwords.txt')
    with open(stopwords_file, "r", encoding="utf-8") as stopword_file:
        stopwords = set(stopword_file.read().splitlines())
        stopwords = stopwords.union(stopwords1)
...
if __name__ == '__main__':
    msg_db.init_database(path=f'{GlobalConfigs().path_database_dir}/MSG.db')
