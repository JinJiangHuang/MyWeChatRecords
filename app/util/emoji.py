...
class Emotion:
    ...
    def init_database(self):
        db_path = os.path.join(GlobalConfigs().path_database_dir, "Emotion.db")
        root_path = os.path.join(GlobalConfigs().path_data_dir, "emoji")
...
def get_emoji(xml_string, thumb=True) -> str:
    """供下载用"""
    try:
        output_path = os.path.join(GlobalConfigs().path_data_dir, "emoji")
...
def get_emoji_path(xml_string, output_path , thumb=True) -> str:
    try:
        emoji_info = parser_xml(xml_string)
        md5 = emoji_info["md5"]
        image_format = [".png", ".gif", ".jpeg"]
