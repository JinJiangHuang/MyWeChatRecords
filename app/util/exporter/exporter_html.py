...
class HtmlExporter(ExporterBase):
    ...
    def image(self, doc, message):
        base_path = os.path.join(GlobalConfigs().path_export_dir, self.contact.get_uniquid_name(), 'image')
        type_ = message[2]
        str_content = message[7]
        str_time = message[8]
        is_send = message[4]
    ...
    def audio(self, doc, message):
        origin_path = os.path.join(os.getcwd(), GlobalConfigs().path_export_dir, self.contact.get_uniquid_name())
        str_content = message[7]
        str_time = message[8]
        is_send = message[4]
        msgSvrId = message[9]
    ...
    def video(self, doc, message):
        ...
        if video_path is None and image_path is not None:
            image_path = path.get_relative_path(image_path, base_path=f'{GlobalConfigs().path_export_dir}/{self.contact.get_uniquid_name()}/image')
            try:
                # todo 网络图片问题
    ...
    def export(self):
        ...
        filename = os.path.join(os.getcwd(), GlobalConfigs().path_export_dir, self.contact.get_uniquid_name(),
                                f'{self.contact.remark}.html')
        print(filename)
        file_path = './app/assets/template.html'
        if not os.path.exists(file_path):
            resource_dir = getattr(sys, '_MEIPASS', os.path.abspath(os.path.dirname(__file__)))
            file_path = os.path.join(resource_dir, 'app', 'assets', 'template.html')
        ...
        for index, message in enumerate(messages):
            ...
            sub_type = message[3]
            timestamp = message[5]
            if (type_ == 3 and self.message_types.__contains__(3)) or (
                    type_ == 34 and self.message_types.__contains__(34)) or (
                    type_ == 47 and self.message_types.__contains__(47)):
                pass
            else:
                # self.progressSignal.emit(1)
                pass

            if type_ == 1 and self.message_types.__contains__(type_):
                self.text(f, message)
            elif type_ == 3 and self.message_types.__contains__(type_):
                self.image(f, message)
            elif type_ == 34 and self.message_types.__contains__(type_):
                self.audio(f, message)
            elif type_ == 43 and self.message_types.__contains__(type_):
                self.video(f, message)
            elif type_ == 47 and self.message_types.__contains__(type_):
                self.emoji(f, message)
            elif type_ == 10000 and self.message_types.__contains__(type_):
                self.system_msg(f, message)
            elif type_ == 49 and sub_type == 57 and self.message_types.__contains__(1):
                self.refermsg(f, message)
            elif type_ == 49 and sub_type == 6 and self.message_types.__contains__(4906):
                self.file(f, message)
            elif type_ == 49 and sub_type == 3 and self.message_types.__contains__(4903):
                self.music_share(f, message)
            elif type_ == 49 and sub_type == 5 and self.message_types.__contains__(4905):
                self.share_card(f, message)
            elif type_ == 49 and sub_type == 2000 and self.message_types.__contains__(492000):
                self.transfer(f, message)
            elif type_ == 50 and self.message_types.__contains__(50):
                self.call(f, message)
            if index % 2000 == 0:
...
class OutputMedia(threading.Thread):
    """
    导出语音消息
    """
    ...
    def run(self):
        ...
        for message in messages:
            ...
            try:
                ...
                logger.error(traceback.format_exc())
        # self.okSingal.emit(34)  # 这里需要替换成实际的信号发射器


class OutputEmoji(threading.Thread):
    """
    导出表情包
    """
...
class OutputImage(threading.Thread):
    """
    导出图片
    """
...
class OutputImageChild(threading.Thread):
    # okSingal = pyqtSignal(int)
    # progressSignal = pyqtSignal(int)
        ...
        for message in self.messages:
            ...
            try:
                ...
                if not os.path.exists(os.path.join(Me().wx_dir, image_path)):
                    ...
                    image_path = image_thumb_path
                image_path = get_image(
                    image_path, base_path=f"/data/聊天记录/{self.contact.get_uniquid_name()}/image"
                )
                try:
                    os.utime(origin_path + image_path[1:], (timestamp, timestamp))
                except:
