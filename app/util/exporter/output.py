...
class Output():
    ...
    def __init__(self, contact, type_=DOCX, message_types={}, sub_type=[], time_range=None, on_export_finish=None):
        super().__init__()
        self.children = []
        self.on_export_finish = on_export_finish
    ...
    def to_csv_all(self):
        ...
        origin_path = os.path.join(os.getcwd(), GlobalConfigs().path_export_dir, '聊天记录')
        os.makedirs(origin_path, exist_ok=True)
        filename = os.path.join(os.getcwd(), 'messages.csv')
        # if not filename[0]:
        #     return
    ...
    def contact_to_csv(self):
        ...
        filename = os.path.join(GlobalConfigs().path_contacts_dir, f'导出联系人_{current_time}.csv')
        # columns = ["用户名", "消息内容", "发送时间", "发送状态", "消息类型", "isSend", "msgId"]
    ...
    def to_docx(self, contact, message_types, is_batch=False):
        ...
        Child = DocxExporter(contact, type_=self.DOCX, message_types=message_types, time_range=self.time_range,
                             on_export_finish=self.on_export_finish)
    ...
    def to_txt(self, contact, message_types, is_batch=False):
        Child = TxtExporter(contact, type_=self.TXT, message_types=message_types, time_range=self.time_range, index=0,
                            on_export_finish=self.on_export_finish)
        # self.children.append(Child)
        # Child.progressSignal.connect(self.progress)
        # if not is_batch:
    ...
    def to_html(self, contact, message_types, is_batch=False):
        ...
        Child.start()
        self.total_num = 1
        if message_types.__contains__(34):
            # 语音消息单独的线程
            self.total_num += 1
            ...
            # output_media.start()
        if message_types.__contains__(47):
            # emoji消息单独的线程
            self.total_num += 1
            ...
            # output_emoji.start()
        if message_types.__contains__(3):
            # 图片消息单独的线程
            self.total_num += 1
    ...
    def run(self):
        print("导出聊天记录格式", self.output_type)
    ...
    def run(self):
        ...
        for message in self.messages:
            ...
            try:
                ...
                image_path = get_image(image_path, base_path=f'/data/聊天记录/{self.contact.get_uniquid_name()}/image')
                try:
                    os.utime(origin_path + image_path[1:], (timestamp, timestamp))
                except:
                    pass
